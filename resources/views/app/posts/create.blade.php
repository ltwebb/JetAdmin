                ...
                <form method="post" action="{{ route('posts.store') }}">
                    ...
                    <div class="bg-white shadow overflow-hidden sm:rounded-md">
                        ...
                        <div class="w-1/2 px-4 py-5 bg-white sm:p-6">
                            <x-label for="image" class="block font-medium text-sm text-gray-700">Email</x-label>
                            <x-input type="file" multiple name="image" id="image" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('image', '') }}" />
                            @error('image')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
